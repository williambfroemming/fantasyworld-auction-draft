...
## Gordon learns that the theme is the plot, and the calendar learns to look forwards

Two pieces of work that turned out to be one. The league's verdict on the Gazette was that it read
as a well-written recap with thematic vocabulary sprinkled over it, and that the target was a
**short story whose plot is generated by what actually happened** — strip the player names and the
scores and a Western should still read as a Western. That is PROMPT_VERSION 13. Making the worlds
vivid then made them collide, which is 14 and 15.

**Learned:**

- **The rule that made immersive fiction compatible with the grounding gate was already there.**
  Fiction wants invented quantities — three hours, ten windows, four doors — and
  `ungroundedNumbers()` rejects any digit not in the pack. *Counts are written as words* had been a
  style note since v1; v13 promotes it to the load-bearing rule, because it is what makes a spelled
  number scenery and a bare digit a claim about the league. Nothing in the gate changed.
- **The same split resolves elapsed time.** "No fortnight, no since October" had to survive contact
  with prose that wants a thing hammering on a door for three hours. Duration *inside the invented
  world* is free; a claim about the real calendar or the order real games arrived in is still
  forbidden. Stating the distinction was enough.
- **GAMENOTES had to be pulled out of the fiction, and that is what let the column go all the way
  in.** The notes are now explicitly the plain record — winner, score, margin, what decided it — so
  a manager can find his own result without reading a short story to do it. Without that pressure
  valve the column could not have been given this much licence.
- **A repair prompt that only says "fix the numbers" gets a worse edition.** Already recorded, and
  it held under v13: week 12 failed the gate on five figures, and the repair message — which
  restates what the edition is *for* — brought back a full court-intrigue piece with a benched
  player left in the corridor holding his hat, not the stat block the naive version produced.
- **"The genre is a suggestion" and "invent a whole world" are incompatible instructions.** v13
  inherited the first and added the second, and the model duly wandered: week eight was assigned
  Halloween Horror and filed folk horror, week nine was assigned Witches and Covens and filed a
  witch trial. Two distinct calendar entries, one indistinguishable world on the page. v14 makes
  the assignment binding with latitude *inside* the world rather than away from it.
- **A backward-looking guard cannot stop a forward-looking theft, and that is not obvious until it
  happens.** `priorLenses` carries the worlds a season has already printed. Week nine was then
  reassigned Superspy Thriller specifically to move it away from the gothic pair — and filed "Cold
  War espionage", which is *week eleven's* assignment, two editions early. `priorLenses` could not
  have caught it, because week eleven did not exist yet. The calendar is fixed and knowable in both
  directions, so v15 ships `reservedGenres`: every other week's assignment, in the pack.
- **Name a genre by its register when a neighbour could swallow it.** "Superspy Thriller" beside a
  reserved "Cold War Espionage" reads as espionage-in-general. The calendar entry is now
  "Superspy Thriller (dinner jackets, a casino, a mountaintop lair, a named villain with a scheme)"
  — the half that le Carré is not.
- **Twice in one session, the natural reading of the data was the wrong one, and only data fixed
  it.** The superflex value bug and this collision were both cases where a comment stating the rule
  did not prevent the rule being broken. A test at the site, or a field in the pack, did. That is
  the argument for `reservedGenres` being a fact rather than another sentence of prompt.

**Watch out for:**

- **A clear-then-rebuild script is destructive the moment it starts.** The regeneration scripts
  delete the range they are about to write, so an interrupt between the two leaves a hole, and the
  deleted issues are unrecoverable if they were never committed — the database rows are gone and
  the mirror file has been rewritten. Commit the archive before regenerating it, or accept that
  stopping halfway means finishing the run.
- **`GENRE_CALENDARS` is immutable per season once published.** Editing `SEASON_ONE` was free only
  because nothing had shipped and everything was being regenerated anyway. After a season's issues
  exist, changing its list silently relabels them under a frame they were never written in. Next
  year is a new key.
- **Weeks 6-8 are v14 and the rest are v15.** Deliberate: their assignments did not change, none of
  them drifted into a reserved world, and regenerating them would have spent three model calls to
  change a stored integer. `prompt_version` is per issue precisely so this stays answerable.
