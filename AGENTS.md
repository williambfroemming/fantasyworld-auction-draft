...
- **"Is this guy hurt" is a stored column, not a fetch.** `players.injury_status` and friends come from the same 5MB Sleeper dump the pool already uses, refreshed out-of-band by `npm run news:refresh` — so on draft night the answer is in Postgres and no provider being down can take it away. **A null status means UNKNOWN, never healthy**: a pool that has never been refreshed has null on every row, and painting those as fit is a confident wrong answer about the exact thing being asked. `InjuryBadge` renders nothing rather than a green tick.
- **Live news is display-only and must stay unreachable from any award path.** `news-service.ts` is the only file that touches a third party: every fetch is caught, timed out at 4s, and degrades to an empty panel. `/api/news` always returns 200 — a dead provider is "no news about this guy", not an error. There are structural tests in `news.test.ts` asserting `draft-service.ts` neither imports it nor calls `fetch(`. The one exception is the open lot's stored injury column on `/api/state`, which is a join that already existed.
